...
namespace VsOutlining.Outlining
{
    ...
    internal class CollapsedHintFormatter
	{
		...
		public string FormatHint()
		{
            ...
            //remove indentation from first line completely
            textLines[0] = textLines[0].TrimStart();

            //for other lines, calculate how much to unindent
			//removing redundant indentation
			//calculating minimal indentation
			int minIndent = int.MaxValue;
            for (int i = 1; i < textLines.Length; i++) {
                minIndent = Math.Min(minIndent, GetIndentation(textLines[i]));
            }
